...
def make_client(use_ollama=False):
    if use_ollama:
...
def compress_text(text, model="gpt-4o", calculate_embeddings=True, use_ollama=False):
    """Compress normal English to caveman compression"""
    client = make_client(use_ollama)

    if use_ollama:
        model = OLLAMA_DEFAULT_MODEL
        calculate_embeddings = False  # ollama doesn't serve the OpenAI embeddings API

    # Detect if this is natural language text
    is_text = is_text_content(text)

    # If it's text, use sentence-by-sentence compression
    if is_text:
        sentences = split_sentences(text)
        sentence_model = model if use_ollama else "gpt-4o-mini"
        ...
        # For code/structured data, compress as whole
        prompt = COMPRESSION_PROMPT.format(text=text)
        response = client.chat.completions.create(
            model=model,
            messages=[
...
def decompress_text(text, model="gpt-4o", use_ollama=False):
...
Examples:
  ...
  # Compress using local ollama (gemma4:26b)
  python caveman_compress.py compress "Your text here" --ollama
  ...
  python caveman_compress.py compress -f input.txt -o output.txt
    ...
    parser.add_argument(
        '-m', '--model',
        default='gpt-4o',
        help='Model to use (default: gpt-4o; ignored when --ollama is set)'
    )
    parser.add_argument(
        '--ollama',
        action='store_true',
        help=f'Use local ollama instead of OpenAI (model: {OLLAMA_DEFAULT_MODEL}, base URL: {OLLAMA_BASE_URL})'
    )
    ...
    # Process text
    model_label = OLLAMA_DEFAULT_MODEL if args.ollama else args.model
    provider_label = f"ollama ({OLLAMA_BASE_URL})" if args.ollama else "openai"

    print(f"\n{'='*60}")
    print(f"MODE: {mode.upper()}")
    print(f"MODEL: {model_label} [{provider_label}]")
    print(f"{'='*60}\n")
    ...
    if mode == 'compress':
        ...
        print("Compressing...\n")
        result, orig_tokens, comp_tokens, reduction, emb_loss, emb_sim = compress_text(
            input_text, args.model, use_ollama=args.ollama
        )
        ...
        print("Decompressing...\n")
        result, cave_tokens, norm_tokens, expansion = decompress_text(
            input_text, args.model, use_ollama=args.ollama
        )

...
\texttt{--reflection-lm} & string & \texttt{ollama/qwen3:} \texttt{4b-instruct-} \texttt{2507-q4\_K\_M} &
...
Buduje referencję API w formacie HTML z wyszukiwarką oraz wszystkie dokumenty
LaTeX projektu w formacie PDF.
...
\begin{itemize}[noitemsep]
  ...
  \item \texttt{docs/\_build/io-2026s-api.pdf} --- PDF referencji API
        (wygenerowany przez Sphinxa przez LaTeX).
  \item \texttt{docs/\_build/io-2026s-userguide.pdf} --- Niniejsza instrukcja
        użytkownika.
  \item \texttt{docs/\_build/io-2026s-problem.pdf} --- Opis problemu i metod
        rozwiązania.
  \item \texttt{docs/\_build/io-2026s-results.pdf} --- Wyniki eksperymentów
        i porównanie z benchmarkiem.
\end{itemize}

Skrypt uruchamia kompilację Sphinxa (HTML i LaTeX) oraz trzy oddzielne
kompilacje \texttt{pdflatex} (po dwa przebiegi każda, dla poprawnego
generowania spisów treści i odsyłaczy).
...
\subsection{Testy automatyczne (pytest)}
...
# Jeden plik
uv run pytest tests/test_classical_agents.py -v

# Jeden test
uv run pytest tests/test_agent.py::test_greedy_agent_solve -v
\end{lstlisting}

\subsection{Testy dymne (sanity scripts)}

Katalog \texttt{sanity/} zawiera cztery skrypty do ręcznego smoke-testowania
komponentów na małych instancjach.
Nie są częścią zestawu pytest --- uruchamia się je bezpośrednio:

\begin{lstlisting}[language=bash]
python sanity/sanity_sch_env.py
python sanity/sanity_agent.py
python sanity/sanity_benchmark.py
python sanity/sanity_classical_agents.py
\end{lstlisting}

Skrypty korzystają z \texttt{data/sch10.txt} (10 zadań) z małą liczbą kroków
(20--30) dla zachowania szybkości. Obejmują: poprawność środowiska,
